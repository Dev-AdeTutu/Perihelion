...
## 11. Anti-Replay, Collision Prevention, and Idempotency (closes #58)

This section is the single authoritative reference for every mechanism that
prevents an intent from being double-locked, double-settled, or confused with
an unrelated intent. Auditors should read this before reasoning about the
security of any state transition.

There are **three distinct mechanisms**, each protecting against a different
threat. They are complementary and must all be correct.

---

### 11.1 The intent nonce — collision prevention (not replay protection)

**Where:** `Intent.nonce` (uint256), generated by `randomNonce()` in
`sdk/src/intent.ts`, signed into the EIP-712 struct, and committed into the
`intent_hash` via the struct hash.

**What it protects against:** _identical-intent collision._ Without a nonce,
two intents for the same user, same asset, same amount, same deadline, and same
destination would produce the same `intent_hash`. The second submission would
silently collide with the first on-chain — the `AlreadyLocked` error would fire
on the EVM escrow, and the second intent would be unserviceable. With a 256-bit
random nonce, the probability of collision is negligible even across many
intents.

**What it does NOT protect against:** transport-layer replay. The nonce is part
of the signed payload and is committed into the hash. An adversary who
intercepts a `lock` transaction on the EVM and retransmits it does not need to
break the nonce — the replay is blocked by the `locks` mapping (§11.3 below),
not by the nonce.

**Reference:** `sdk/src/intent.ts` → `randomNonce()`,
`PerihelionEscrow.sol` → `INTENT_TYPEHASH` (nonce field in the struct).

---

### 11.2 The LayerZero transport nonce — message-replay prevention
...
intent nonce (§11.1). Nor does the transport nonce prevent the same intent from
...
### 11.3 The `locks` mapping and Settled/Cancelled markers — double-lock and double-settle prevention

These are the application-layer idempotency guards. They are independent of the
transport nonce and survive transport-layer failures or replays.

#### Double-lock prevention (EVM)

`locks[intentHash].user != address(0)` means the intent is already locked.
`PerihelionEscrow.lock` checks this and reverts with `AlreadyLocked` if set.
Because `intentHash` includes the user's intent nonce (§11.1), two distinct
intents can never share the same lock slot even if their other fields are
identical.

#### Double-settle / double-cancel prevention (EVM)

`Lock.released` and `Lock.refunded` are boolean flags set to `true` on first
settlement or refund respectively. `_onFillConfirmed` and `_onCancelIntent`
check `l.released || l.refunded` and revert with `AlreadyFinalized` if either
is set. `cancelExpired` performs the same check. This guarantees exactly one
terminal transition per lock slot, regardless of how many `FillConfirmed` or
`CancelIntent` messages arrive (including replays — though replays are already
blocked by §11.2, this is belt-and-suspenders).

#### Double-settle / double-cancel prevention (Soroban)

`DataKey::Settled(intent_hash)` and `DataKey::Cancelled(intent_hash)` are
persistent storage markers. They are:

- Written **before** the outbound dispatch in `fill_intent` and
  `cancel_expired_intent` (effects before interactions, standard CEI pattern).
- Checked at the top of every state-mutating entrypoint via `is_finalized` (or
  the explicit marker checks in `cancel_expired_intent`).
- Stored in **persistent** tier so they survive ledger archival — a late
  FillInstruction re-delivery cannot re-open a settled intent even if the
  `IntentRecord` entry has been archived.

The `ConfirmationSent(intent_hash)` marker prevents double-dispatch of the
FillConfirmed message (relevant for the `deliver_intent` + `dispatch_confirmation`
two-step path).

#### Double-registration prevention (Soroban)

`on_fill_instruction` checks `env.storage().persistent().has(&key)` before
writing a new `IntentRecord`. If the key already exists (either a live record or
a finalized one), the registration is silently skipped. This makes FillInstruction
delivery idempotent — a LayerZero message re-delivery (which would have the same
nonce, already blocked by §11.2, but hypothetically) would not overwrite an
in-progress or completed intent.

---

### 11.4 Which mechanism prevents what — summary table
...
| Same intent cancelled twice on Soroban | `Cancelled` marker + `IntentStatus` check (§11.3) | `cancel_expired_intent`, `on_cancel_inbound` |
| FillConfirmed dispatched twice | `ConfirmationSent` marker (§11.3) | `dispatch_confirmation` |
| Intent re-registered after settlement/cancellation | `is_finalized` check + persistent marker read (§11.3) | `on_fill_instruction` |

An intent can be double-settled **only** if **all three** of the following fail
simultaneously:
1. The transport nonce guard fails (LayerZero endpoint bug or DVN compromise),
2. AND the application-layer idempotency marker is absent (persistent storage
   corrupted or the flag was never written due to a reentrancy exploit — both
   prevented by CEI ordering and `nonReentrant`),
3. AND the lock flag check fails (EVM) or the in-memory status check fails
   (Soroban).

The combination of independent, layered guards means no single component failure
can cause double-settlement.
